...
/**
 * Verifies that the generated JSON is valid.
 */
public class TOMLJSONSerializationTest {
    ...
    public void testShouldSerializeToJSONProperly(String testName) throws Exception {
        ...
        try (StringWriter sw = new StringWriter(32 * 1024)) {
            toml.writeJSON(sw);
        ...
        // Must equal the expected object
        JsonNode diff = JsonDiff.asJson(expected, generated);

        // .. then the comparison should be an empty array
        assertTrue(diff.isArray());
        boolean diffIsEmpty = ((ArrayNode) diff).isEmpty();
        if (!diffIsEmpty) {
            String message = String.format("Expected an empty difference for generated JSON for file: %s but got:"
                    + "%nEXPECTED:%n"
                    + "%s"
                    + "%nGENERATED:%n"
                    + "%s"
            ...
            System.out.println("The generated JSON was:");
